@echo off
rem Launch OpenXcom with the AI Bridge TCP server enabled.
rem Extra args are forwarded to openxcom.exe.

setlocal
set PORT=12345
set ROOT=%~dp0
set EXE=%ROOT%build\bin\Release\openxcom.exe
if not exist "%EXE%" set EXE=%ROOT%build\bin\Debug\openxcom.exe
if not exist "%EXE%" set EXE=%ROOT%bin\x64\Release\OpenXcom.exe
if not exist "%EXE%" (
  echo ERROR: openxcom.exe not found under build\bin\{Release,Debug} or bin\x64\Release.
  echo Build the game first ^(see README^), or edit EXE= in this script.
  exit /b 1
)

rem CD into the exe's directory so OpenXcom finds standard\ and UFO\ data folders.
for %%I in ("%EXE%") do set EXEDIR=%%~dpI
cd /d "%EXEDIR%"
"%EXE%" -ai-server %PORT% %*
